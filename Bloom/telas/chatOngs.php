...
$id_ongs = intval($_SESSION['id_usuario']);


    $stmt = $conn->prepare("SELECT nome FROM ongs WHERE 
    id_ongs = ?");
...
if(!empty($comentario)){
       $stmt = $conn->prepare("INSERT INTO comentarios (id_evento, id_ongs, comentario) VALUES (?,?,?) ");
     $stmt->bind_param("iis",$id_evento, $id_ongs,$comentario );
   $stmt->execute();
...
<button type="submit"><i class="bi bi-send"></i></button><br>
    ...
    <div class="localbtn">
      <form  action="../validacoes/telaOngs.php" method="post">
             <?php
                                echo '<input type="hidden" name="id_usuario" value="' .$_SESSION['id_usuario'].  '">';
                                ?>
    <button class="sair" type="submit">Voltar</button>
  </form>
    </div>
</body>
</html>
